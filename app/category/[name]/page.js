...
export default function CategoryPage(){
  ...
  useEffect(()=>{
    const load=async()=>{
      ...
      // subCategory hrang hrang lak chhuah
      const unique = [...new Set(filtered.map(s=> s.subCategory).filter(Boolean))]
      setSubCats(unique)
      setLoading(false)
    }
    load()
  ...
  return(
    <div style={{minHeight:'100vh', background: dark?'#121212':'#f2f2f7', paddingTop:'70px'}}>
      ...
      <div style={{padding:'10px 14px'}}>
        <h2 style={{fontSize:'22px', fontWeight:'800', color: dark?'white':'black', marginBottom:'14px'}}>{decoded}</h2>
        {subCats.length===0 && <div style={{color: dark?'#aaa':'#666'}}>Sub Category la awm lo</div>}
        <div style={{display:'flex', flexDirection:'column', gap:'10px'}}>
          {subCats.map(sc=>(
            <div key={sc} onClick={()=>router.push(`/series/${encodeURIComponent(sc)}`)} style={{background: dark?'#1e1e1e':'white', padding:'18px', borderRadius:'16px', border: dark?'1px solid #333':'1px solid #eee', cursor:'pointer', display:'flex', justifyContent:'space-between', alignItems:'center'}}>
              <span style={{fontWeight:'700', color: dark?'white':'black'}}>{sc}</span>
