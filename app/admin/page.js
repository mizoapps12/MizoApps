...
export default function Admin(){
  ...
  const [tab,setTab]=useState('write') // write or manage
  ...
  useEffect(()=>{ if(user) loadCats() },[user, tab])
  ...
  const delCat=async(id)=>{ if(confirm('Delete Category?')){ await deleteDoc(doc(db,'categories',id)); loadCats() } }
  ...
  return(
    <div style={{minHeight:'100vh', background:'#f2f2f7'}}>
      {/* TAB HEADER - A LANG NGEI TUR - WHITE BG A CHUNG AH */}
      <div style={{background:'white', padding:'12px', display:'flex', gap:'10px', borderBottom:'2px solid #eee', position:'sticky', top:'0', zIndex:50}}>
        <button onClick={()=>setTab('write')} style={{flex:1, padding:'14px', borderRadius:'12px', border:tab==='write'?'2px solid #111':'1px solid #ddd', background:tab==='write'?'#111':'white', color:tab==='write'?'white':'#111', fontWeight:'800', fontSize:'14px'}}>
          ✍️ Write Story
        </button>
        <button onClick={()=>setTab('manage')} style={{flex:1, padding:'14px', borderRadius:'12px', border:tab==='manage'?'2px solid #111':'1px solid #ddd', background:tab==='manage'?'#111':'white', color:tab==='manage'?'white':'#111', fontWeight:'800', fontSize:'14px'}}>
          📚 Manage Category
        </button>
      </div>
        ...
        <div style={{width:'92%', maxWidth:'420px', margin:'0 auto'}}>
          ...
          {tab==='write' && (
            <div style={{display:'flex', flexDirection:'column', gap:'14px'}}>
              <h2 style={{fontWeight:'800', textAlign:'center', margin:'10px 0'}}>Story Thar Ziahna</h2>
              <input placeholder="Thupui / Title" value={form.title} onChange={e=>setForm({...form,title:e.target.value})} style={box}/>

              {/* Category Option - Manage a i siam mil in auto lang */}
              ...
              {/* Sub Category - Category thlan a sub a neih chuan alang ang */}
          ...
          {tab==='manage' && (
            <div>
              <h2 style={{fontWeight:'800', textAlign:'center', margin:'10px 0'}}>Category Manage</h2>
              <p style={{textAlign:'center', fontSize:'11px', color:'#888', marginBottom:'14px'}}>Tah hian i siam mil zel in Write Story ah auto in a lang ang</p>
              ...
              {categories.map(c=>(
                <div key={c.id} style={{background:'white',padding:'14px',borderRadius:'14px',marginBottom:'12px',border:'1px solid #eee'}}>
                  {editId===c.id? (
                    ...
                    <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                      ...
                      <div style={{display:'flex',gap:'6px'}}>
                        <button onClick={()=>startEdit(c)} style={{background:'#e8f0fe',color:'#111',border:'none',padding:'7px 12px',borderRadius:'8px',fontSize:'12px',fontWeight:'700'}}>Edit</button>
                  ...
                  <div style={{display:'flex',gap:'6px',marginTop:'12px'}}>
                    ...
                    <button onClick={()=>addSub(c.id)} style={{background:'#ff6b00',color:'white',border:'none',padding:'0 14px',borderRadius:'10px',fontWeight:'700', fontSize:'12px'}}>Add Sub</button>
                  </div>
                  <div style={{display:'flex',gap:'6px',flexWrap:'wrap',marginTop:'10px'}}>
                    {(c.subcategories||[]).map((s,i)=><span key={i} style={{background:'#f2f2f7',padding:'6px 12px',borderRadius:'20px',fontSize:'11px', display:'flex', alignItems:'center', gap:'6px'}}>{s} <span onClick={()=>delSub(c.id,s)} style={{background:'#ff3b30', color:'white', width:'18px', height:'18px', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', fontWeight:'800'}}>x</span></span>)}
                  </div>
                </div>
              ))}
